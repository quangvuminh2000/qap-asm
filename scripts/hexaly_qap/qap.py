...
def hexaly_qap(F: np.ndarray, D: np.ndarray, max_time:int=5) -> Tuple[List[int], int]:
    n = F.shape[0]
    ...
    with hexaly.optimizer.HexalyOptimizer() as optimizer:
        ...
        # Create F as an array to be accessed by an at operator
        array_F = model.array(F)

        # Minimize the sum of product distance*flow
        obj = model.sum(D[i][j] * model.at(array_F, p[i], p[j]) for j in range(n) for i in range(n))
        model.minimize(obj)
        model.close()

        # Parameterize the optimizer
        optimizer.param.verbosity = 0
        optimizer.param.time_limit = max_time
