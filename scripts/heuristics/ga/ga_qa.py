...
mutation_strategy = Mutation(
    mutation_algorithm=BasicMutation(), mutation_probability=MUTATION_PROBABILITY
)
crossover_strategy = Crossover(
    crossover_algorithm=BasicCrossover(), crossover_probability=CROSSOVER_PROBABILITY
)
...
def ga_qa(F, D):
    ...
    for generation in tqdm(
        range(NUMBER_OF_GENERATIONS), desc="Generation", leave=False
    ):
        ...
        selected_chromosomes = selection_strategy.select(
    ...
    return population[np.argmin(fitness_scores)], max_results[-1]
