...
def delta_swap(F, D, p, i, j):
    ...
    n = len(p)
    pi, pj = p[i], p[j]
    d = (F[i, i] - F[j, j]) * (D[pj, pj] - D[pi, pi]) + (F[i, j] - F[j, i]) * (
        D[pj, pi] - D[pi, pj]
    )
    for k in range(n):
        if k in [i, j]:  # skip the swap
            continue
        pk = p[k]
        d += (F[i, k] - F[j, k]) * (D[pk, pj] - D[pi, pk]) + (F[k, i] - F[k, j]) * (
            D[pj, pk] - D[pi, pk]
        )
    return d
...
def has_qa(
    F, D, n_ants=10, iters=100, R_ratio=1 / 3, q_exploit=0.9, alpha_evap=0.1, Q=100
):
    n_ants = int(n_ants)
    iters = int(iters)
        ...
        for k in range(n_ants):
            ...
            for _ in range(R):
                ...
                if random.random() < q_exploit:
                    ...
                    probs = np.array(
                        [
                            tau[r, p[s]] + tau[s, p[r]] if s != r else 0
                            for s in range(n)
                        ],
                        dtype=np.float64,
                    )
